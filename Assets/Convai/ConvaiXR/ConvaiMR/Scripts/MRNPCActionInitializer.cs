...
public class MRNPCActionInitializer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _convaiInteractablesData = FindFirstObjectByType<ConvaiInteractablesData>();
        _navMeshBuilder = FindFirstObjectByType<NavMeshBuilder>();
    }
    ...
    private IEnumerator InitializeAction(List<MRUKAnchor> MRUKAnchors)
    {
        ...
        ConvaiNPC[] convaiNPCs = FindObjectsByType<ConvaiNPC>(FindObjectsSortMode.None);
    ...
    private void UpdateActionObjects(List<MRUKAnchor> MRUKAnchors)
    {
        ...
        {
            if (new LabelFilter(mrukAnchor.Label).PassesFilter(_sceneLabels))
            {
